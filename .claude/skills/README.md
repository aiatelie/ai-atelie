...
- **`canvas-sync/`** — keeps the DesignCanvas surface coherent across the canonical starter (`mcp/starters/DesignCanvas.jsx`), the byte-identical demo mirror (`web/projects/demo/design-canvas.jsx`), and the host wiring (`web/src/lib/tweakBridge.ts` + `mcp/CANVAS_PROTOCOL.md`). Auto-fires on diffs to any of those four. Encodes the parity rule (md5sum-verified), the flow direction (canonical → mirror, never the other way), and the protocol-doc-updated-in-the-same-commit rule for `__dc_*` and `__page_is_canvas` changes. Without this, fixes silently land in only one of two places — demo tests stale code, or new projects get content nobody verified.
