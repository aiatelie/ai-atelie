 ...
 * that came from computing turn shape independently in 3+ places.
 *
 * This module is intentionally dependency-free (no React, no Node) so it
 * can run server-side (replay/tests) and, in Phase 4, frontend-side from
 * one shared implementation. Phase 0 deliverable: these pure functions +
 * an invariant test suite (canonicalEvents.test.ts).
 *
 ...
 * Block id rule: providers that stream content-block indices use those;
 * whole-item providers mint `${streamId}:${seq}` (handled at append time,
 * api/src/services/canonicalLog.ts — Phase 1).
 */
