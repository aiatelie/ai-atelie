...
export const CAPABILITIES: Capability[] = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        resolution: { type: "string", enum: ["1080p", "1440p", "4K", "8K", "custom"], description: "Target output resolution. Default '1080p'." },
        ...
        quality: { type: "string", enum: ["draft", "standard", "high", "master"], description: "draft=H.264 CRF 28; standard=CRF 23; high=CRF 18; master=ProRes 4444 if transparent else H.264 CRF 14. Default 'standard'." },
  ...
  },
  {
    ...
    description:
      "Bundle a specific element as an EBU OGraf HTML graphics package (.ograf.zip) for DaVinci Resolve 21+ Media Pool. Captures HTML, CSS, fonts, and referenced images. Returns an artifact envelope. NOTE: Resolve 21 imports OGraf as a baked clip — text fields are NOT post-import editable in the Inspector. For editable templates suggest PNG (re-export when changed) or wait for the Fusion .setting capability.",
    inputSchema: {
      type: "object",
      properties: {
        projectId: { type: "string" },
        ...
        editableTitle: { type: "boolean", description: "Stamp the first <h1>/<h2> with data-ograf-title so the OGraf updateAction can change it. Default false." },
      },
      required: ["projectId", "route", "selector"],
    },
    method: "POST",
