...
async function buildSandboxPrompt(
  ...
  lines.push(
    ...
    `  • **\`window.claude.complete(promptOrOptions)\`** is available inside every artifact — call it to ask Claude (Haiku 4.5, max 1024 tokens) and \`await\` a string back. Pass a prompt string, or \`{ messages: [{role,content}] }\`. Use it for AI-powered interactions: graded quizzes, dynamic feedback, content generation. Rate-limited to ~10 calls/min, ~30s timeout. Don't put an API key in the artifact — the host proxies the call.`,
    ``,
