...
export default function Onboard() {
  ...
  useEffect(() => {
    ...
    const intake = [
      ...
      ``,
      `Call \`mcp__ask-user__ask_user\` ONCE with a batched form covering the artifact type AND the standard intake checklist. Pass { title, questions: [...] } where each question has { id, kind, title, options? }. Don't add "Decide for me" / "Explore a few" / "Other" yourself — the form auto-injects them.`,
      ``,
      `First question — \`{ id: "artifact_type", kind: "enum", title: "What kind of artifact is this?", options: [...] }\` — to pin down the medium. Use options like:`,
      ...
      ``,
      `Then 5-8 follow-up questions in the same batched form covering the intake checklist:`,
      `  • output details — size, aspect ratio, fidelity (\`kind: "enum"\` or \`"number"\`)`,
      `  • option count — how many variations, across which dimensions (\`kind: "number"\`)`,
      `  • constraints — timing, copy length, platform rules, accessibility (\`kind: "text"\`, multiline)`,
      `  • design system / brand context — existing tokens, references to upload, or starting fresh (\`kind: "enum"\` or \`"file"\`)`,
      ``,
      ...
      ``,
      `When you've gathered enough to start, end your final reply with the marker [READY_TO_BUILD] on its own line. The UI watches for that marker and auto-opens the editor for you — don't ask me to click anything. Batched form first, conversational follow-ups only if something genuinely needs nuance after I've answered. Don't read the starter files yet.`,
    ].join("\n");
